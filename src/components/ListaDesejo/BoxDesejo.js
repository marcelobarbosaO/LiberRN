...
export default class BoxDesejo extends Component {
    ...
    _validaClick(){
        if(this.props.item.id_pub != "0" && this.props.item.id_pub != 0){
            Actions.AnuncioScreen({ item: this.props.item.publicacao_data });
        }
    }
    ...
    render() {
        return (
            <View style={sty.box}>
                <TouchableHighlight onPress={ () => this._validaClick() } style={sty.boxBg} underlayColor="transparent">
                    <View style={sty.boxBg}>
                        ...
                        <View style={{ flex: 3, flexGrow: 3, width: 0, position: 'relative' }}>
                            ...
                            <Text style={sty.dataDesejo}>Criado em {this.props.item.data}</Text>
...
const sty = StyleSheet.create({
    ...
    dataDesejo: { fontFamily: font, fontSize: (Platform.OS == 'ios') ? 12 : 14, color: "#424b54", paddingBottom: 10 },
