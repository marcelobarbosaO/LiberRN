...
class NovoDesejo extends Component {
    constructor(props) {
        super(props);
        this.userProfile = JSON.parse(this.props.profile);
        this.state = { titulo: '', imageLivro: '', tela: 1, loadLivros: false, listaImages: [], width: 0, height: 0, imageKey: '' };
    ...
    _saveDesejo(){
        if(this.state.imageLivro == ''){
            ...
            axios.post('http://liberapp.com.br/api/save_desejo',{ user_id: this.userProfile.server_response.server_id, texto: this.state.titulo, foto: this.state.imageLivro})
            .then((response) => {
        ...
        return (
            <View style={[est.boxGeral]} >
                <View style={est.ToolBar}>
                    <TouchableHighlight onPress={() => { Actions.ListaDesejoScreen({type: ActionConst.RESET}) }} underlayColor="#FFF" style={{ flex: .7, paddingLeft:15 }}>
                        <View style={{ flex: 1, justifyContent:'center' }}>
                            <Icon name="md-arrow-round-back" size={20} color="#2B3845" />
                        </View>
                    </TouchableHighlight>
                    <View style={{ flex: 1, alignItems: "center" }}>
                        <Text style={{ fontSize: (Platform.OS == 'ios') ? 15 : 17, color: '#2B3845', fontWeight: 'bold' }}>Novo Desejo</Text>
                    </View>
...
export default connect(mapStateToProps, {})(NovoDesejo);